...
StateGraph(AgentState) ──▶ Plan-Act 外层状态图
    ...
    │       └──▶ error_handler ──▶ 容错检查（术语归一/纠正/兜底/消歧/槽位）
    │               ├──▶ 需澄清 ──▶ generate_clarification ──▶ END
    │               └──▶ 通过 ──▶ classify_complexity ──▶ LLM 判断复杂度
...
- **独立容错节点 `error_handler`**：在 `classify_complexity` 之后插入，作为不侵入主逻辑的独立 state_node。负责对用户输入进行质量检查：
  - **术语归一化**：基于预定义 `TERM_MAP`（20+ 常见 AI 术语），正则替换口语/简写为标准术语（如 SAM → Segment Anything Model）。
  - **纠正意图检测**：基于 `NEGATIVE_SIGNALS` 规则检测 13 种负面信号词（"不对/错了/应该是/我说的是"等），触发上下文回滚确认。
  - **双层置信度评估**：第一层为启发式规则（输入长度 ≤3 → 0.15；含模糊词 → 0.35），置信度 < 0.25 直接快速兜底，不消耗 LLM；第二层为 LLM + `ErrorHandlerResult`（Pydantic 结构化输出），对边界情况做精细判断。
  - **槽位补全与意图消歧**：定义 `paper_title/author/year/concept/topic/query` 等关键槽位，缺失时生成自然追问；多意图冲突时生成选项让用户选择。
  - **状态可追溯**：`AgentState` 新增 `pending_clarification`（待澄清消息）、`error_log`（纠错事件列表）、`last_slots`（已确认槽位）、`confidence`（最近一次置信度），由 `SqliteSaver` 自动持久化。
  - **追问消息生成**：`generate_clarification` 节点将 `pending_clarification` 包装为 `AIMessage` 返回给用户，随后直接走到 `END`，跳过工具执行。
  - **manage_memory 自动清除**：每次用户发送新消息时，`manage_memory` 将 `pending_clarification` 设为 `None`，确保上一轮澄清状态不会干扰新请求。
- **状态定义 `AgentState`**：在原有 `messages`（`Annotated[list, add_messages]`）基础上扩展 `plan`、`current_step_index`、`step_results`、`need_replan`、`is_complex`、`final_summary`、`replan_count`、`conversation_summary`、`summary_covered_rounds`，以及容错字段 `pending_clarification`、`error_log`、`last_slots`、`confidence`。
...
│  ├── 纠正意图检测：NEGATIVE_SIGNALS 匹配   │
│  │   （"不对/错了/应该是/我说的是"...）     │
│  └── 启发式置信度评估                      │
│      ├── 输入 ≤3 字 → confidence=0.15     │
│      ├── 含模糊词 → confidence=0.35       │
│      └── 其他 → confidence=0.8            │
│      若 confidence < 0.25 → 直接兜底       │
└──────────────────────────────────────────┘
    │
    ▼（规则层未触发时进入 LLM 层）
┌──────────────────────────────────────────┐
│  LLM 层（Pydantic 结构化输出）             │
│  ├── ErrorHandlerResult 解析              │
│  │   ├── needs_clarification: bool        │
│  │   ├── clarification_type: str          │
│  │   │   （confidence_low / disambiguation│
│  │   │     / slot_filling / correction    │
│  │   │     / none）                       │
...
└──────────────────────────────────────────┘
    ...
    ▼
更新 AgentState：
├── pending_clarification → 追问消息（或 None）
├── confidence → 置信度
├── error_log → 追加本次纠错事件
└── last_slots → 合并新提取的槽位
```
...
- **双层检查架构**：规则层（零 LLM 开销）+ LLM 层（精细判断）。规则层处理明确的极端情况（极短输入、纠正信号），置信度 < 0.25 直接快速兜底；LLM 层负责边界判断（意图消歧、槽位缺失）。
- **可扩展术语映射**：`TERM_MAP` 以字典形式集中管理，新增术语只需添加键值对，无需改动逻辑代码。正则采用 `\b...\b` 词边界 + `IGNORECASE`，避免误替换子串。
- **纠正意图的上下文回滚**：检测到负面信号词时，自动清空 `last_slots`，因为用户正在否定上一轮理解，已确认的槽位不应继续保留。
- **Pydantic 结构化输出**：`ErrorHandlerResult` 统一定义所有容错判断字段，与 `agent.py` 中已有的 `ComplexityJudgment`、`DeviationCheck` 等模型风格一致，保持项目代码统一性。
- **零侵入主逻辑**：`error_handler` 作为独立节点插入图结构，不修改 `classify_complexity`、`react_agent`、`generate_plan` 等任何现有节点的代码，仅通过条件路由决定是否继续执行。

**对外接口**：

| 函数 | 作用 |
|------|------|
| `create_error_handler_node(llm)` | 返回一个 LangGraph state_node 函数，接收 state 返回状态更新字典 |
| `ErrorHandlerResult` | Pydantic 模型，定义容错判断的完整字段 |
...
用户输入消息
    ...
    │               │       └── 侧边栏渲染 📋 执行计划卡片
    │               ├──▶ error_handler (pending_clarification)
    │               │       └── 显示"🤔 我不太确定你的意思，让我确认一下..."
    │               │       └── 清空计划卡片（本次不执行工具）
...
- **执行计划卡片 `_render_plan_card`**：在侧边栏动态渲染计划步骤，使用 Streamlit 原生组件区分状态：
  ...
  - 未开始 → 普通 markdown（灰色 ⚪）
- **Loading 状态提示**：根据 Agent 所处阶段（制定计划、执行步骤、重规划、生成回答、容错追问），在主聊天区上方显示动态 `st.info()` / `st.warning()` 提示，让用户感知当前处理进度。当 `error_handler` 触发 `pending_clarification` 时，显示"🤔 我不太确定你的意思，让我确认一下..."并立即清空侧边栏计划卡片（本次不会执行工具）。
- **容错追问的流式展示**：`generate_clarification` 节点生成的追问消息通过 `messages` 模式正常流式输出，前端无需额外处理即可逐字显示。`updates` 模式中的 `generate_clarification` 节点状态用于更新 Loading 提示为"💬 正在向你确认..."。
- **计划状态持久化**：通过 `st.session_state["execution_plan"]` 保存最终计划结果，页面刷新（rerun）后执行计划卡片仍然保留在侧边栏，不会消失。发送新消息时自动清空上一次计划。当触发容错时，计划状态被显式清空，避免追问消息与过期计划卡片同时展示。
...
graph TB
    ...
    subgraph AGENT["智能体调度层"]
        ...
        ERROR_H["error_handler<br/>容错检查 + 术语归一"]
        GEN_CLAR["generate_clarification<br/>生成追问消息"]
